...
# ---------- DATABASE CONNECTION ----------
__db = cntr.connect(host='localhost',
                    user='RyunRandhawa',
                    passwd='R005911',
                    database='book_shop')
__cur = __db.cursor()
...
def unique_book_no():
    ...
    data = __cur.fetchall() # fetchone() is sufficient since we only need one row
...
def delete_stock():
    ...
    __cur.execute(f"DELETE FROM purchased WHERE Book_no = {bno}")
        # Then, delete the row from the `stock` table
    __cur.execute(f"DELETE FROM stock WHERE Book_No = {bno}")
